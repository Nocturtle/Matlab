...
sett=struct;
sett.auxttldur=0.5;% aux TTL will go high at start of isi, then low after this duration
...
sett.staticdur=1;% duration that the stimulus will present, but not move
sett.sdur=2;% duration that the stimulus will move after inital presentation
...
sett.seg=4;% split the stimulus into this many segments (number of white/black bar pairs, max=126?)
...
bcm2835_gpio_fsel(ttl_pin_aux, 1);
bcm2835_gpio_clr(ttl_pin_aux)
%get screen info ----------------------------------------------------
...
bak=testOrder;
if sett.staticdur>0, testOrder=[strcat(bak,'_static'),testOrder]; end
if sett.rev, testOrder=[testOrder,strcat(bak,'_reversal')]; end
...
% ISI ---------------------------------------------------------------------
    ...
    if sett.isidur>0
bcm2835_gpio_set(ttl_pin_aux);
    t0=GetSecs; t1=t0;
    abstime(j)=t0-ts;
    while(t1-t0<sett.isidur) && flag
        t1=GetSecs;
        if t1-t0<sett.auxttldur
          bcm2835_gpio_clr(ttl_pin_aux);
        end
...
% stim ------------------------------------------------------------------
    ...
    t0=GetSecs; t1=t0;
    abstime(j)=t0-ts;
    while(t1-t0<sett.staticdur) && flag
        t1=GetSecs;
        if KbCheck, flag=false; break; end
    end, j=j+1;
    end
